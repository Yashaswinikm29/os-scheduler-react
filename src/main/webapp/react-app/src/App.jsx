...
import { fcfs, sjf, prioritySched, roundRobin } from './scheduler.js'
...
const ALGO_COLORS = ['#1a6b8a','#7c3aed','#0e7c7b','#b85c00']
const BEST_FOR    = ['Batch systems','Known burst times','Critical priority tasks','Interactive systems']
const RESOURCE    = ['Very Low','Low','Medium','Medium–High']
...
export default function App() {
  ...
  const runSim = () => {
    ...
    const q = quantum || 2
    const r = algo==='fcfs' ? fcfs(procs) : algo==='sjf' ? sjf(procs) : algo==='priority' ? prioritySched(procs) : roundRobin(procs, q)
    setResult(r)
  ...
  const runCompare = () => {
    ...
    const q = cmpQ || 2
    const results = [fcfs(cmpProcs), sjf(cmpProcs), prioritySched(cmpProcs), roundRobin(cmpProcs, q)]
    let bestIdx = 0
      ...
      <header style={css.header}>
        <div style={css.headerInner}>
          ...
          <div style={css.badges}>
            {['● SYSTEM ONLINE','FCFS','SJF','PRIORITY','ROUND ROBIN'].map((b,i)=>(
          ...
          <>
            <div style={css.panel}>
              ...
              <div style={css.formRow}>
                ...
                {algo==='rr' && (
                  <div style={css.formGroup}>
                    <label style={css.label}>Time Quantum</label>
                    <input style={css.input} type="number" min="1" value={quantum} onChange={e=>setQuantum(+e.target.value)}/>
                  </div>
                )}
              </div>
            </div>
          ...
          <>
            <div style={css.panel}>
              ...
              <div style={{...css.formRow,marginBottom:14}}>
                <div style={css.formGroup}>
                  <label style={css.label}>Time Quantum (Round Robin)</label>
                  <input style={css.input} type="number" min="1" value={cmpQ} onChange={e=>setCmpQ(+e.target.value)}/>
                </div>
                <button style={{...css.btnCompare,marginTop:18}} onClick={runCompare}>Compare All 4 Algorithms</button>
              </div>
              <ProcessTable
...
function CompareResults({ results, bestIdx }) {
  const reasons = [
    ...
    `Round Robin ensures fair CPU distribution with no starvation.`
  ]
...
function FormulasTab() {
  const formulas = [
    ...
    ['Round Robin Cycles',      'Cycles = ⌈Burst Time ÷ Quantum⌉',        'Number of times a process is scheduled before completion in Round Robin.'],
  ]
  return (
    <>
      <div style={css.panel}>
...
function GuideTab() {
  const algos = [
    ...
    { name:'Round Robin', color:'#b85c00', type:'Round Robin · Preemptive',          desc:'Each process gets equal CPU time (quantum). Preempts after quantum expires.', pros:['Fair','No starvation','Best response'], cons:['Context switch overhead'], best:'Interactive & time-sharing systems', avoid:'Quantum too small', cpu:'Medium–High' },
  ]
  ...
  const whenToUse = [
    ...
    ['Interactive / time-sharing system','Round Robin','#b85c00','Every process gets fair CPU time'],
    ...
    ['Minimize average waiting time','SJF','#7c3aed','Proven optimal for non-preemptive scheduling'],
    ['Prevent starvation','Round Robin','#b85c00','All processes guaranteed CPU time'],
